...
class ReplayMemory:
    ...
    def sample(self, batch_size, device):
        ...
        states = torch.Tensor([list(x) for x in np.asarray(transitions)[:, 0]]).to(device)
        actions = torch.Tensor([list(x) for x in np.asarray(transitions)[:, 1]]).to(device)
        rewards = torch.Tensor([list(x) for x in np.asarray(transitions)[:, 2]]).to(device)
        next_states = torch.Tensor([list(x) for x in np.asarray(transitions)[:, 3]]).to(device)
        dones = torch.Tensor([list(x) for x in np.asarray(transitions)[:, 4]]).to(device)
...
class DDPG:
    def __init__(self, args):
        ...
        ## TODO ##
        self._actor_opt = torch.optim.Adam(self._actor_net.parameters(), lr=1e-3) # gradient descent toward behavior network instead of target network
    ...
    def select_action(self, state, noise=True):
        ...
        ## TODO ##
        if noise:
            eps = torch.from_numpy(self._action_noise.sample()).to(self.device)
        else:
...
def main():
    ...
    parser.add_argument('-m', '--model', default='ModelWeights/ddpg.pth')
